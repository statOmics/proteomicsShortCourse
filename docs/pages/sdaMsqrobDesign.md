...
## 2.3 Evaluating the impact of sample size
...
### 2.4.2. Evaluation of contrasts of interest. We would like to perform all pairwise comparisons between each of the treatments. How many contrasts do we need for this, specify each contrast

### 2.4.3. Interpret the log 2 transformed fold change for the top hit of the comparison involving treatment B vs A.

### 2.4.4. What do you observe for the comparisons involving treatment D and/or E? Can you explain this?

##2.5. Blocking: Heart dataset

 Doll et al. 2018 (DOI: 10.1038/s41467-017-01747-2) established the healthy human heart proteome of 16 distinct anatomical regions in 3 trauma victims.  The proteome was analysed using a quadrupole Orbitrap mass spectrometer and searched with MaxQuant version 1.5.5.6 against the 2015 Uniprot human databases (UP000005640_9606 and UP000005640_9606_additional).
 In this exercise we use a subset of the data: proteoms of the right atrium (RA), right ventricle (RV), the left atrium (LA) and left ventricle (LV) and the atrial and ventricular septa (SepA and SepV) separating the atria and ventricles.
 location
 The researchers are interested in assessing if the proteome on average differs between

 1. The atrium and ventricular regions, i.e. comparing the average expression of the three atrium regions (RA, LA, SepA) vs the average of the three ventricular regions (RV, LV, SepV).
 2. The left and the right atrium
